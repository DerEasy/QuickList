...
public:
    ...
    void addMiddle(Node<T>* node, Node<T>* nextNode, T data) {
        node->setData(data);
    ...
    void addFromFront(int index, Node<T>* node, Node<T>* nextNode, T data) {
        int i = 0;

        while (this->hasNext(nextNode)) {
            if (i == index) {
                this->addMiddle(node, nextNode, data);
                return;
            }
    ...
    void addFromBack(int index, Node<T>* node, Node<T>* nextNode, T data) {
        int i = getSize() - 1;

        while (hasPrev(nextNode)) {
            if (i == index) {
                addMiddle(node, nextNode, data);
                return;
            }
